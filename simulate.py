...
def main(cfg: DictConfig) -> None:
    ...
    output_image_dir = "simulation_renders"
    ...
    while True:
        ...
        if cfg.simulation.visualization.visualize:
            ...
            cv2.imwrite(os.path.join(output_image_dir, f"frame_{frame_count:04d}_rgb.png"), rgb)
            
            logging.info(f"RGB image saved to: {os.path.join(output_image_dir, f'frame_{frame_count:04d}_rgb.png')}")
            
            cv2.imwrite(os.path.join(output_image_dir, f"frame_{frame_count:04d}_depth.png"), depth * 255.0)
            
            logging.info(f"Depth image saved to: {os.path.join(output_image_dir, f'frame_{frame_count:04d}_depth.png')}")
