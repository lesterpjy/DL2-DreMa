...
class SurfInteractiveGaussianModel(InteractiveGaussianModel):
    ...
    def add(self, gaussian_model):
        ...
        self._scaling = torch.cat((self._scaling, gaussian_model._scaling[:, :2]))
    ...
    def save_original_ply(self, path):
        ...
        # add a 0 to the end of the scaling vector to match the ply format
        scale = np.concatenate((scale, np.zeros((scale.shape[0], 1))), axis=1)
        rotation = self._rotation.detach().cpu().numpy()

        list_of_attributes = self.construct_list_of_attributes()
        # add the scale_2 attribute after scale_1 attribute
        list_of_attributes.insert(list_of_attributes.index('scale_1') + 1, 'scale_2')
