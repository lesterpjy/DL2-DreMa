...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if depth_folder is not None:
            ...
            depths = np.load(depth_path) if os.path.exists(depth_folder) else None
        else:
            depth_path = None
            depths = None
        '''
...
def readColmapSceneInfo(path, images, eval, depth_folder, llffhold=8):
    try:
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), depth_folder=os.path.join(path, depth_dir))
    ...
    if not os.path.exists(ply_path):
        print("Converting point3d.bin to .ply, will happen only the first time you open the scene_grouping.")
        try:
